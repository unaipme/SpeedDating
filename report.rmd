...
- `[d_]{attractive|sincere|intelligence|funny|ambitious}`: A score from 0 to 10 representing the opinion subject 1 has of themselves about the five criteria.
- `[d_]{attractive|sincere|intelligence|funny|ambitious|shared_interests}_partner`: A score from 0 to 10 representing the opinion subject 1 made of subject 2 after the date.
...
There are several works done based on this dataset. [-@Dataset] Most significant work done is the initial reason of the creating the dataset, an academic work with title "Gender Differences in Mate Selection: Evidence from a Speed Dating Experiment". Authors of the work are same people with the ones who gathered the dataset. [-@FirstPaper] They wanted to investigate the selectivity factors for individuals and for different genders. Apart from this work, other examples contains different interpretation for the dataset. One of the popular ones is [-@SecondOne], it exploits the results in order to visualize the importance of features for participants.  
...
Next, the results and conclusions for the four different predictors that we tried. These are the MLP articial neural network, rests, support vector machines (SVM) and logistic regression.
...
When plotting the importance of the each variables, we found out that the feature with the highest importance (by a significant gap) was `like`, which expresses whether the subject liked their date, out of 10. This feature can be seen as a posteriori feature, kind of a summary of the rest of features. Noise aside, it could be seen as a linearly dependent feature. So, we decided to try to build another random forest without the bias of the `like` feature, and the result can be seen in figure \ref{fig:rf-var-importance}. We think no one will be shocked by how attractiveness and shared interests play such an important role in deciding the outcome of a date. But the most important characteristic seems to be whether the partner is funny or not. Now, that is more of a surprise.

Table \ref{tab:confusion-matrix-rf} shows the confusion matrix obtained by the prediction, which scores a PPV of `r round(conf.matrix.rf$byClass[3], 3)` and a NPV of `r round(conf.matrix.rf$byClass[4], 3)`, which tells us obviously that this is a very reliable model, the most so far. However, what the table does not show is the OOB-error, which is at `r round(rev(best.rf.model.2$finalModel$err.rate[,1])[1] * 100, 2)`%, even though it does not show with the our test set.
...
And now comes the twist: The data is taken from an actual couple, which means that the prediction should be 1 in all cases. As can be seen in table \ref{tab:new-data-pred}, all models return the same outcome for both rows. And what is better is that most of them also predict correctly, except for SVM. However, with such a small new population, this shall not be considered as any kind of validation error.
...
knitr::kable(round(final.results, 3), format="latex", booktabs=T, caption="Final comparison of results on validation set")
```
